<template>
  <div class="shop_container">
    <ul class="shop_list">
      <li class="shop_li border-1px" v-for="item in shoplists" :key="item.id">
        <router-link :to="`/shop/goods/`+item.id">
          <div class="shop_left" v-if="item.logo">
            <img class="shop_img" v-lazy="`https://sqqproject.oss-cn-hangzhou.aliyuncs.com/`+ item.logo"/>
          </div>
...
export default {
  ...
  // 获取数据
  // async mounted() {
  //   this.shoplists = await shoplists()
